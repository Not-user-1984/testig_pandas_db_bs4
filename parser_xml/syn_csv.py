...
# Фиксированный путь к CSV-файлу
CSV_FILE_PATH = "parser_xml/results_csv/spimex.cvs"

BATCH_SIZE = 100  # Размер пакета для вставки данных
...
def process_csv_file(connection):
    ...
    with open(CSV_FILE_PATH, mode="r", encoding="utf-8") as file:
        ...
        batch = []
        for row in reader:
            # Проверяем, что все необходимые колонки присутствуют и не пусты
            if not all(row.get(key) for key in ["exchange_product_id", "exchange_product_name", "oil_id"]):
...
def main():
    ...
    try:
        ensure_table_exists(connection)  # Убедимся, что таблица существует
